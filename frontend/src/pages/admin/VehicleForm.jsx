...
import { Upload, X, ArrowLeft, Save, Trash2, FileText, Plus } from 'lucide-react';
...
const VehicleForm = () => {
    ...
    const [existingTechnicalSheet, setExistingTechnicalSheet] = useState(null);
    ...
    const [financingFeatures, setFinancingFeatures] = useState([]); // Dynamic key-value pairs
    ...
    const fetchVehicleData = async () => {
        try {
            ...
            setFormData({
                ...
                price: vehicle.price ? Math.round(parseFloat(vehicle.price)).toString() : '',
                currency: vehicle.currency || 'USD',
                mileage: vehicle.mileage !== undefined && vehicle.mileage !== null ? vehicle.mileage : '',
            ...
            if (vehicle.pricing_details && vehicle.pricing_details.custom_financing_features) {
                setFinancingFeatures(vehicle.pricing_details.custom_financing_features);
            }
            if (vehicle.technical_sheet_url) {
                setExistingTechnicalSheet(vehicle.technical_sheet_url);
            }
        } catch (error) {
    ...
    // Helper to parse formatted string like "25.000.000,50" -> 25000000.50
    const parsePrice = (priceStr) => {
        if (!priceStr) return 0;
        const normalized = String(priceStr).replace(/\./g, '').replace(',', '.');
    ...
    // Financing Logic
    const calculatePlan = () => {
        const price = parsePrice(formData.price) || 0;
    ...
    const addFinancingFeature = () => {
        if (financingFeatures.length >= 8) {
            Swal.fire('Atención', 'Puede agregar un máximo de 8 características.', 'warning');
            return;
        }
        setFinancingFeatures([...financingFeatures, { label: '', value: '' }]);
    };

    const updateFinancingFeature = (index, field, value) => {
        const newFeatures = [...financingFeatures];
        newFeatures[index][field] = value;
        setFinancingFeatures(newFeatures);
    };

    const removeFinancingFeature = (index) => {
        setFinancingFeatures(financingFeatures.filter((_, i) => i !== index));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        Object.keys(formData).forEach(key => {
            if (key === 'price') {
                data.append(key, parsePrice(formData[key]));
            } else {
        ...
        const pricingDetails = {
            ...
            custom_financing_features: financingFeatures
        };
    ...
    <AdminLayout>
        <div className="max-w-4xl mx-auto pb-12">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
                <h1 className="text-3xl font-bold text-white">{isEditMode ? 'Editar Vehículo' : 'Crear Nuevo Vehículo'}</h1>
                <p className="text-gray-400 mt-1">Completa los detalles descriptivos y técnicos.</p>
            </div>
          </div>
          ...
          {isFetchingData ? (
             <div className="flex justify-center flex-col items-center py-20 gap-4">
                 ...
                 <p className="text-gray-400">Cargando datos del vehículo...</p>
             </div>
          ) : (
          ...
          <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 space-y-6">
            <h3 className="text-xl font-bold text-white border-b border-gray-700 pb-2">Ficha Técnica (PDF Opcional)</h3>
            
            {isEditMode && existingTechnicalSheet && (
                 <div className="mb-4 flex items-center gap-3 bg-gray-700/50 p-3 rounded-lg border border-gray-600">
                     <FileText className="text-accent" />
                     <a href={`${API_BASE}${existingTechnicalSheet}`} target="_blank" rel="noopener noreferrer" className="text-white hover:text-accent transition font-medium">
                         Ver Ficha Técnica Actual
                     </a>
                     <p className="text-xs text-gray-400 ml-auto hidden sm:block">Si subes un nuevo archivo, se reemplazará.</p>
                 </div>
            )}

            <div className="border-2 border-dashed border-gray-600 rounded-xl p-6 text-center hover:border-accent transition-colors relative">
              <input
                type="file"
                accept="application/pdf"
                onChange={(e) => {
                    if (e.target.files && e.target.files.length > 0) {
                        setTechnicalSheet(e.target.files[0]);
                    }
                }}
                className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
              />
              <FileText className="mx-auto text-gray-400 mb-2" size={32} />
              <p className="text-gray-300 font-medium z-10 relative pointer-events-none">
                  {technicalSheet ? technicalSheet.name : 'Arrastra un PDF o haz clic para subir'}
              </p>
              <p className="text-gray-500 text-sm mt-1">Soporta solo formato .PDF</p>
            </div>
            {technicalSheet && (
                 <button type="button" onClick={() => setTechnicalSheet(null)} className="text-red-400 hover:text-red-300 text-sm flex items-center justify-center gap-1 mt-3 w-full">
                     <X size={16} /> Quitar archivo seleccionado
                 </button>
            )}
          </div>
          ...
          <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 space-y-6">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="relative">
                    <label className="block text-gray-400 mb-2 text-sm">Precio</label>
                    <div className="relative">
             ...
             {/* Custom Financing Features */}
             <div className="pt-4 border-t border-gray-700">
                 <div className="flex justify-between items-center mb-4">
                     <h4 className="text-lg font-semibold text-white">Financiación / Tasas (Opcional)</h4>
                     <button
                        type="button"
                        onClick={addFinancingFeature}
                 ...
                 {financingFeatures.length === 0 ? (
                      <p className="text-gray-500 text-sm italic">No hay características de financiación adicionales.</p>
                 ) : (
                      <div className="space-y-3">
                          {financingFeatures.map((feature, index) => (
                              <div key={index} className="flex flex-col sm:flex-row gap-4 items-center bg-gray-700/30 p-3 rounded-lg border border-gray-600/50">
                                  <div className="flex-1 w-full">
                                      <label className="block text-[10px] uppercase text-gray-400 mb-1 ml-1">Etiqueta</label>
                                      <input
                                          type="text"
                                          placeholder='Ej: "Plan 70/30"'
                                          value={feature.label}
                                          onChange={(e) => updateFinancingFeature(index, 'label', e.target.value)}
                                          className="w-full bg-gray-800 text-white rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-accent outline-none border border-transparent focus:border-accent/50"
                                      />
                                  </div>
                                  <div className="flex-1 w-full">
                                      <label className="block text-[10px] uppercase text-gray-400 mb-1 ml-1">Valor</label>
                                      <input
                                          type="text"
                                          placeholder='Ej: "$50.000"'
                                          value={feature.value}
                                          onChange={(e) => updateFinancingFeature(index, 'value', e.target.value)}
                                          className="w-full bg-gray-800 text-white rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-accent outline-none border border-transparent focus:border-accent/50"
                                      />
                                  </div>
                                  <button
                                      type="button"
                                      onClick={() => removeFinancingFeature(index)}
                                      className="text-red-400 hover:text-red-300 p-2 sm:mt-5 bg-red-400/10 hover:bg-red-400/20 rounded-lg transition"
                                  >
                                      <Trash2 size={18} />
                                  </button>
                              </div>
